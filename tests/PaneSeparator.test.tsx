...
test("moves by keyboard, reaches both limits and resets with Enter", () => {
  ...
  const values: number[] = [];
  let resets = 0;
  render(<PaneSeparator label="Resize tree" value={248} min={208} max={360} onMove={(delta) => moves.push(delta)} onSet={(value) => values.push(value)} onReset={() => { resets += 1; }} />);
  const separator = screen.getByRole("separator", { name: "Resize tree" });
